...
| E16 | Chang-Zhu-Tan, 4e réseau de validation externe de h(D) | **FAIT** — corrigé 2026-08-24 (commits `4367e19`, `466046c`, `770f083` ; entraînement terminé, prédiction P1 réfutée ; 252 lignes `city=czt` dans `raw_results.csv`). h(D) = **0.469** (jeu complet, valeur finale utilisée dans le manuscrit depuis le 2026-08-24 — pas 0.413, qui venait d'un calcul train-only incohérent avec Beijing/London/Madrid ; cf. `PREREGISTRATION_CZT.md` §9). Cette ligne marquait encore « collecte de données en cours » alors que l'entraînement était terminé — cf. `configs/experiments/e16_czt_validation.yaml` |
